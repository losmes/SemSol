...
class FineSemSol(nn.Module):
    def __init__(self, args, config_class, model_class, args_bert_model, tokenizer_len=0):
        super().__init__()

        bertconfig = config_class.from_pretrained(args_bert_model, num_labels=1)
        bertconfig.output_hidden_states = True
        self.bert = model_class.from_pretrained(args_bert_model, config=bertconfig)
        ...
        bertconfig_1 = config_class.from_pretrained(args_bert_model, num_labels=1)
        self.bert_1 = model_class.from_pretrained(args_bert_model, config=bertconfig_1)
    ...
    def forward(self, input_ids, input_mask, segment_ids, utter_ids,
                mask_noun, masks_noun, ids_noun,
                mask_adj, masks_adj, ids_adj, masks_noun_ids, masks_adj_ids):
        ...
        _, context_output_hidden = self.bert(inputs_embeds=inputs_embeds, 
        ...
        output = self.bert_1(inputs_embeds=last_heads, 
                             attention_mask=input_mask, 
                             token_type_ids=segment_ids)
        return output
